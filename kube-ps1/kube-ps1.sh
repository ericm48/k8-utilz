...
kube_ps1() {
  ...
  [[ -n "${KUBE_PS1_PREFIX}" ]] && KUBE_PS1+="$PWD ${KUBE_PS1_PREFIX}"

  #echo "PromptBe1: ${KUBE_PS1}"
...
#
  ...
  if [[ "${KUBE_PS1_NS_ENABLE}" == true ]]; then
    ...
    KUBE_PS1+="$(_kube_ps1_color_fg ${KUBE_PS1_NS_COLOR})${KUBE_PS1_NAMESPACE}${KUBE_PS1_RESET_COLOR}${KUBE_PS1_SEPARATOR}"
  fi

  #echo "PromptBe3: ${KUBE_PS1}"
